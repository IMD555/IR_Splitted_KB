...
const uint16_t keymap[] = {
	...
	ESC_, _F1_, _F2_, _F3_, _F4_, _F5_, _F6_, _F7_, //_F8_, // _F9_, F10_, F11_, F12_, ____, ____, ____, ____, //0x0n
	BSP_, APOS, __1_, __2_, __3_, __4_, __5_, __6_, //__7_, __8_, // __9_, __0_, MIN_, EQU_, ____, ____, ____, ____, //0x0n
	P_UP, TAB_, __Q_, __W_, __E_, __R_, __T_, __Y_, //__U_, // __I_, __O_, __P_, BSP_, GRAV, BR_L, BR_R, ____, ____, ____, ____, //0x1n
	P_UP, CAPS, __A_, __S_, __D_, __F_, __G_, ____, //__H_, __J_, // __K_, __L_, DEL_, ENT_, ____, ____, ____, ____, //0x2n
	P_DN, SHLL, __Z_, __X_, __C_, __V_, __B_, ____, //__N_, __M_, // COMM, DOT_, C_UP, SHRL, ____, ____, ____, ____, //0x3n
	P_DN, ____, CTLL, GULL, ALLL, _SP_, FN_L, ____, // FNRL, C_LT, C_DN, C_RT, ____, ____, ____, ____, //0x4n
	//	CONN, SHDN, _SP_, SEMI, SLAS, BSLA, PREV, PLAY, NEXT, V_DN, V_UP, ____, ____, ____, ____, //0x5n
};
...
void APP_KeyboardTasks(void)
{
	...
	if (HIDTxHandleBusy(keyboard.lastINTransmission) == false) {
		...
		//			//			static uint8_t count = 0;
		//			//			static uint8_t report[7] = {0, 0, 0, 0, 0, 0, 0};
		//
		//			uint8_t data = (bufferRX[1] << 3) | (bufferRX[2]&7);
		//			if (bufferRX[2] < 8 && data < 48) {
		//				data = 47 - data;
		//				if (getKeyGroup(keymap[data]) == KEY_NORMAL) {
		//					inputReport.keys[0] = keymap[data];
		//					//				} else if (getKeyGroup(keymap[data]) == KEY_MODIFIER) {
		//					//					inputReport.modifiers.value |= 1 << (data & 0x07);
		//					//				} else {
...
static void APP_KeyboardProcessOutputReport(void)
{
	...
	if (outputReport.leds.numLock) {
		LED_On(LED_USB_DEVICE_HID_KEYBOARD_KANA);
	} else {
		LED_Off(LED_USB_DEVICE_HID_KEYBOARD_KANA);
	}
}
