...
void main(void)
{
 ...
 while (1) {
  ...
  static uint8_t countZeroPushing = 3 +1;
  static uint8_t countData = 0;
  static uint8_t dataA[32];

  if (countZeroPushing > 3) {
  ...
  while ((irRxBuffer.readIndex != irRxBuffer.writeIndex)) {
  ...
  if (fSendReport) {
   fSendReport = 0;
   if (nPushing || countZeroPushing <= 3) {
    uint8_t reportCopy[1 + 3];

    TMR5IE = 0;
    for (uint8_t c = 0; c < 1 + 3; c++) {
     reportCopy[c] = report[c];
    }
    TMR5IE = 1;

    sendReport(0x4, &reportCopy[0], nPushing);
    if (!nPushing) {
     countZeroPushing++;
    }
   }
