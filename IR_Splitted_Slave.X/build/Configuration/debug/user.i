...
void sendReport(uint8_t command, uint8_t* data, uint8_t dataCount)
{
 ...
 for (uint8_t c = 0; c < dataCount; c++) {
  dataBuf[c * 2 + 1] = dataBuf[c * 2] + (5 + ((data[c] >> 3) * 3));
  dataBuf[c * 2 + 2] = dataBuf[c * 2 + 1] + (5 + ((data[c] & 7) * 3));
 }

 dataBuf[dataCount * 2 + 1] = dataBuf[dataCount * 2] + (5 + ((checksum(command,&data[0],dataCount)) * 3));

 dataBuf[dataCount * 2 + 2] = dataBuf[dataCount * 2 + 1] + ((5 + ((10) * 3)));
 ...
 CCP1CONbits.MODE = 0x09;
 T2CONbits.ON = 1;
 CCP1CONbits.MODE = 0x02;
